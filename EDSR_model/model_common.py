...
class ResBlock(nn.Module):
    def __init__(self, n_feats, kernel_size, padding, bias=True, bn=False, act=nn.ReLU(True), res_scale=1):
        super(ResBlock, self).__init__()
        m = []

        for i in range(2):
            m.append(nn.Conv2d(n_feats, n_feats, kernel_size, padding, bias=bias))
            if bn:
